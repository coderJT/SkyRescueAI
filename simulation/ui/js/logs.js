...
export function initLogs(state, utils, mcpClient) {
    ...
    function addThought(droneIdx, type, msg) {
        ...
        const isMCP = type === 'mcp-call' || type === 'mcp-resp';
        ...
        thoughtLogs[droneIdx].push({ id: thoughtId, type, msg: formattedMsg, time: Date.now(), isMCP, isLLM, global: false });
        trimLog(droneIdx);
    }
    ...
    function summariseResponse(name, resp) {
        ...
        if (name === 'scan_sector') {
            const haz = resp.hazard || resp.status || 'unknown';
    ...
    function addMCP(droneIdx, callType, toolName, args, response) {
        ...
        thoughtLogs[droneIdx].push({ type: 'mcp-call', msg: callMsg, time: Date.now(), isMCP: true, isLLM: false, global: false });
        thoughtLogs[droneIdx].push({ type: 'mcp-resp', msg: respMsg, time: Date.now() + 1, isMCP: true, isLLM: false, global: false });
        trimLog(droneIdx);
